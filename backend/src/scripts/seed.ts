...
const SEED_USERS: SeedUser[] = [
  // ── Admin (hidden from contact selector) ────────────────────────────────────
  {
  ...
  // ── Managing Partners (contact selector order) ───────────────────────────────
  {
    ...
    linkedinUrl:  '',
    displayOrder: 1,   // ← appears first among partners
  },
  {
    ...
    for (const user of SEED_USERS) {
      ...
      await client.query(
        `INSERT INTO users (
          ...
          title, bio, specialty, linkedin_url, display_order, is_active
        ) VALUES (
